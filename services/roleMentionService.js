...
async function collectRoleMentionShowData(client, kv, guildId) {
    const settings = await kv.hGetAll(
        roleMentionTargetsKey(guildId),
    );
    ...
    for (const [roleId, targetId] of Object.entries(settings)) {
        ...
        const customMessage = await kv.hGet(
            roleMentionMessageMapKey(guildId),
            roleId,
        );

        validLines.push(
            `ロール: <@&${roleId}> → 転載先: <#${targetId}> / カスタム文面: ${customMessage ? 'あり' : 'なし'}`,
        );
    }
...
async function applyRoleMentionCleanup(kv, guildId, staleEntries) {
    ...
    for (const entry of staleEntries) {
        const key = `${entry.roleId}:${entry.targetId}:${entry.type}`;
        ...
        const removedLine = await applySingleRoleMentionCleanup(
            kv,
            guildId,
            entry,
        );
