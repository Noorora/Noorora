...
function forumMessageMapKey(guildId) {
  return `forum-log-message-map:${guildId}`;
}
...
async function main() {
    ...
    try {
      ...
      if (interaction.commandName === 'forum') {
        ...
        if (sub === 'channel') {
          ...
          if (messageTemplate) {
            await kv.hSet(forumMessageMapKey(interaction.guildId), forum.id, messageTemplate);
          }
        ...
        if (sub === 'show') {
          ...
          const messageTemplates = await kv.hGetAll(forumMessageMapKey(interaction.guildId));
          ...
          const lines = Object.entries(settings).map(([forumId, targetId], index) => {
            const hasCustomMessage = Boolean(messageTemplates[forumId]);
            return `${index + 1}. フォーラム: <#${forumId}> → 通知先: <#${targetId}> / カスタム文面: ${hasCustomMessage ? 'あり' : 'なし'}`;
          });
        ...
        if (sub === 'unset') {
          ...
          const deletedTarget = await kv.hDel(guildMapKey(interaction.guildId), forum.id);
          const deletedMessage = await kv.hDel(forumMessageMapKey(interaction.guildId), forum.id);

          if (!deletedTarget && !deletedMessage) {
            await interaction.reply({
  ...
  client.on(Events.ThreadCreate, async (thread) => {
    try {
      ...
      const customTemplate = await kv.hGet(forumMessageMapKey(thread.guildId), thread.parentId);
